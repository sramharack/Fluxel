...
from flowforge_mep.water_flows import (
    project_calculation,
...
SEED_JSON = DATA_DIR / "ursuline_convent_seed_project.json"

st.set_page_config(page_title="FlowForge MEP — Water Flows", page_icon="💧", layout="wide")
...
def load_seed_project():
    ...
    project["metadata"].update({
        ...
        "purpose": "Demo project used to validate FlowForge calculations against the spreadsheet."
    })
    return project
...
st.title("FlowForge MEP — Water Flows")
...
with st.sidebar:
    ...
    st.download_button(
        ...
        file_name="flowforge_water_flows_project.json",
...
with tabs[4]:
    st.subheader("Clean site flow diagram")
    if results is None:
        ...
        st.caption("Block-flow representation separates potable water supply/storage from wastewater generation/storage/disposal. This replaces the rough Sankey chart with a cleaner engineering-style flow diagram.")
